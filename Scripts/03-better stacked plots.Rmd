...
df$monthyear <- df %>% ym(date) ## not working - want month and date in the x axis!
...
colorsb <- c("#0072b2","#d55e00",  "#009e73",  "#f0e442") # scott color blind friendly

## Recoding NA to undetermind
df$race_rec <- df$race_rec %>%
  fct_explicit_na("Undetermined")

df %>% ggplot(aes(x=year, fill=race_rec)) +
  geom_bar(col = 'white') +
  scale_fill_manual(values = ggpubfigs::friendly_pal("ito_seven")) +
  labs(
    x = 'Year',
    y = 'Counts',
...
ggsave(filename = here("Output", "year-race.png"), bg="white")

## highlight the "undetermined"
df %>% ggplot(aes(x=year, fill=race_rec)) +
  geom_bar(col = 'white') +
  scale_fill_manual(values = ggpubfigs::friendly_pal("bright_seven")) +
  labs(
    x = 'Year',
    y = 'Counts',
    title = 'Fatal Police Shootings by Year and Race',
  ...
  gghighlight(race_rec == 'Undetermined', use_direct_label = F)

ggsave(filename = here("Output", "year-race-undetermined.png"), bg="white")
